...
<head>
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '/' + appLocale + '/translations/view/' + $(this).val();
                } else {
                    window.location.href = '/' + appLocale + '/translations';
                }
            });
            ...
            document.querySelector('.group-select').addEventListener('change', function () {
                window.location.href = '/' + appLocale + '/translations/view/' + this.value;
            });
    ...
    <script>
        const appLocale = "<?php echo app()->getLocale(); ?>";
    </script>
...
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="{{ action('\Barryvdh\TranslationManager\Controller@getIndex', ['locale' => app()->getLocale()]) }}" class="navbar-brand">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    ...
    <form method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup', ['locale' => app()->getLocale()]) ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
        ...
        <table class="table">
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@postDelete', ['locale' => app()->getLocale(), $group, $key]) ?>">
                               class="delete-key"
